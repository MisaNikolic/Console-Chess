...
//  setting up Kings
    ...
    public ArrayList<Square> getWhitePieces() {
        ArrayList<Square> whitePieces = new ArrayList<>();
        for (int i = 0; i < 8; i++)
            for (int j = 0; j < 8; j++)
                if (squares[i][j].getPiece() != null && squares[i][j].getPiece().getColor().equals("white")) {
                    whitePieces.add(squares[i][j]);
                }
        return whitePieces;
    }

    public ArrayList<Square> getBlackPieces() {
        ArrayList<Square> blackPieces = new ArrayList<>();
        for (int i = 0; i < 8; i++)
            for (int j = 0; j < 8; j++)
                if (squares[i][j].getPiece() != null && squares[i][j].getPiece().getColor().equals("black")) {
                    blackPieces.add(squares[i][j]);
                }
        return blackPieces;
    }
...
// going "down-left"
    ...
    public int numberOfAttackersAOnSquare(Square square, String opponentsColor) {
        int attackers = 0;
        int i1 = square.getIndex().getX(), j1 = square.getIndex().getY();
        String myColor = opponentsColor.equals("white") ? "black" : "white";
//  checking for possible L-shaped attacks on that square
        if (i1 + 2 < 8 && j1 + 1 < 8 && isPieceOnSquare(i1 + 2, j1 + 1, new Knight(opponentsColor))) attackers++;
        if (i1 + 1 < 8 && j1 + 2 < 8 && isPieceOnSquare(i1 + 1, j1 + 2, new Knight(opponentsColor))) attackers++;
        if (i1 + 2 < 8 && j1 - 1 > 0 && isPieceOnSquare(i1 + 2, j1 - 1, new Knight(opponentsColor))) attackers++;
        if (i1 + 1 < 8 && j1 - 2 > 0 && isPieceOnSquare(i1 + 1, j1 - 2, new Knight(opponentsColor))) attackers++;
        if (i1 - 2 > 0 && j1 + 1 < 8 && isPieceOnSquare(i1 - 2, j1 + 1, new Knight(opponentsColor))) attackers++;
        if (i1 - 1 > 0 && j1 + 2 < 8 && isPieceOnSquare(i1 - 1, j1 + 2, new Knight(opponentsColor))) attackers++;
        if (i1 - 2 > 0 && j1 - 1 > 0 && isPieceOnSquare(i1 - 2, j1 - 1, new Knight(opponentsColor))) attackers++;
        if (i1 - 1 > 0 && j1 - 2 > 0 && isPieceOnSquare(i1 - 1, j1 - 2, new Knight(opponentsColor))) attackers++;

        ArrayList<Square> opponentsPieces;
        if(opponentsColor.equals("white"))
        opponentsPieces = getWhitePieces();
        else
        opponentsPieces = getBlackPieces();

        for(int i = 0; i < opponentsPieces.size(); i ++) {
                if(opponentsPieces.get(i).getPiece().canMoveToASquare(square, opponentsPieces.get(i), this, null))
                    attackers ++;
        }
    ...
    public Square[] attackersOnSquare(Square square, String color) {
        Square[] attackers = new Square[numberOfAttackersAOnSquare(square, color)];
        int current = 0;
        int i1 = square.getIndex().getX(), j1 = square.getIndex().getY();
//  checking for possible L-shaped attacks on that square
        ...
        if (attackDirection(square, color).equals("vertical-up")) {
            while (squares[i +k][j] ==null) {
                if (isSquareInCheck(squares[i +k][j], opponentColor)) {
                    if(k==1 && numberOfAttackersAOnSquare(squares[i +k][j], opponentColor) >1)
                        return true;
                    if(k>1)
                        return true;
                }
        ...
        if (attackDirection(square, color).equals("horizontal-left")) {
            while (squares[i][j-k] ==null) {
                if (isSquareInCheck(squares[i][j-k], opponentColor)) {
                    if(k==1 && numberOfAttackersAOnSquare(squares[i][j-k], opponentColor) >1)
                        return true;
                    if(k>1)
                        return true;
                }
        ...
        if (attackDirection(square, color).equals("diagonal-upright")) {
            while (squares[i +k][j+k] ==null) {
                if (isSquareInCheck(squares[i +k][j+k], opponentColor)) {
                    if(k==1 && numberOfAttackersAOnSquare(squares[i +k][j+k], opponentColor) >1)
                        return true;
                    if(k>1)
                        return true;
                }
